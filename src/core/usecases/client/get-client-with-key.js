...
function getClientWithKey(configuration, repositories) {
  ...
  return async (apikey) => {
    ...
    try {
        const current_client = await repositories.client.findOneBy('api_key', decryptData(key, algorithm, apikey));
...
module.exports = getClientWithKey;